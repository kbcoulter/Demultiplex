...
import gzip
import bioinfo.py # IMPORTS convert_phred (Set to 33 encoding)
...
def get_args():
     ...
     parser.add_argument("-o", "--outname", help="An name for the output histogram <outname>.png", required=True, type=str)
     return parser.parse_args()

args = get_args()

##### SCRIPT BODY #####
qscores = np.zeros(args.readsize, dytpe = float) # Will hold summed qscores then the mean after div.
num_records = 0

with gzip.open(args.filename, "rt") as fastq: # TESTING GZIP OPEN... Work?
